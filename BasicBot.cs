...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class BasicBot : IBot
    {
        ...
        public const string BooksIntent = "ChooseBook";
                        ...
                        case DialogTurnStatus.Empty:
                            switch (topIntent)
                            {
                                ...
                                case BooksIntent:
                                    ...
                                    var filesList = FileShare.GetFilesList();
                                    List<CardAction> Buttons = new List<CardAction>();
                                    foreach (var item in filesList)
                                    {
                                        Buttons.Add(new CardAction() { Title = item, Type = ActionTypes.ImBack, Value = item });
                                    }
                                    HeroCard card = new HeroCard("Choose book:", "Choose the book you want to query from. The books displayed are from your Azure Storage account", null, null, Buttons, null);
                                    Activity replyActivity = activity.CreateReply();
                                    replyActivity.Attachments = new List<Attachment>() { card.ToAttachment() };
                                    await dc.Context.SendActivityAsync(replyActivity);
                                    break;
                                case NoneIntent:
                                default:
                                    if ((activity.Text.Split('.').Last() == "txt") || (activity.Text.Split('.').Last() == "docx") || (activity.Text.Split('.').Last() == "pdf"))
                                    {
                                        FileShare.fileName = activity.Text;
                                        await dc.Context.SendActivityAsync($"Your book has been set to {activity.Text}.\nPlease ask query.");
                                        break;
                                    }
                                    else
                                    {
                                        string result = FileShare.Retrieve();

                                        var values = new Dictionary<string, string>
                                            {
                                               { "qas_id", "1" },
                                               { "question", luisResults.Text },
                                               { "context", result },
                                            };

                                        var parameters = new FormUrlEncodedContent(values);

                                        var response = await client.PostAsync("http://127.0.0.1:5000/mrc", parameters);

                                        var responseString = await response.Content.ReadAsStringAsync();
                                        var answer = JObject.Parse(responseString)["Answer"].ToString();
                                        await dc.Context.SendActivityAsync(answer);
                                        break;
                                    }
                            }
